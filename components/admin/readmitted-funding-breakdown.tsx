...
interface ReadmittedFundingBreakdownProps {
  data: {
    selfFunded: number
    nsfas: number
  }
...
export function ReadmittedFundingBreakdown({ data, onOpenDrilldown }: ReadmittedFundingBreakdownProps) {
  const chartData = [
  ...
  return (
    <Card className="overflow-hidden rounded-2xl border border-border/60 bg-card/95 shadow-[0_18px_45px_rgba(15,23,42,0.12)] backdrop-blur">
      <CardHeader className="border-b border-border/60 bg-gradient-to-r from-muted/70 via-muted/40 to-transparent dark:from-muted/40 dark:via-muted/20 dark:to-transparent">
        <CardTitle className="text-xl font-semibold text-foreground">Readmitted Students Funding</CardTitle>
        <CardDescription className="text-muted-foreground">Funding distribution for readmitted students</CardDescription>
      </CardHeader>
        ...
        <div className="space-y-6">
          ...
          >
            <div className="flex items-center justify-between">
              <span className="text-sm font-medium text-foreground">Total Readmitted Students</span>
