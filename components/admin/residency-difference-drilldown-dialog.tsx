...
interface ResidencyDifferenceDrilldownDialogProps {
  open: boolean
  onOpenChange: (open: boolean) => void
  students: Learner[]
  data: { onCampus: number; offCampus: number; difference: number }
}
...
export function ResidencyDifferenceDrilldownDialog({
  open,
  onOpenChange,
  students,
  data,
}: ResidencyDifferenceDrilldownDialogProps) {
  ...
  const subtitle = `Difference: ${data.difference >= 0 ? "+" : ""}${data.difference} (On-campus − Off-campus)`

  return (
    <StudentBreakdownDrilldownDialog
      open={open}
      onOpenChange={onOpenChange}
      title="Residency Difference"
      subtitle={subtitle}
      segments={segments}
      students={atRisk}
