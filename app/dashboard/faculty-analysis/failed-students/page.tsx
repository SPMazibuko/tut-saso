...
export default function FailedStudentsPage() {
    ...
    const fetchFailedStudents = async () => {
      try {
        ...
        const response = await fetch(
          `/api/faculty-analysis/failed-students?year=${yearFilter}&qualification=${qualificationFilter}&campus=${campusFilter}`
        )
  ...
  const filteredStudents = useMemo(() => {
    return failedStudents.filter((student) => {
      const matchesSearch =
        ...
        student.moduleCode.toLowerCase().includes(searchTerm.toLowerCase()) ||
        (student.moduleName && student.moduleName.toLowerCase().includes(searchTerm.toLowerCase())) ||
  ...
  useEffect(() => {
    setCurrentPage(1)
  }, [searchTerm, yearFilter, campusFilter, qualificationFilter])
  ...
  const displayedRangeStart = filteredStudents.length === 0 ? 0 : (currentPage - 1) * ITEMS_PER_PAGE + 1
  ...
  const handlePrint = () => {
    exportToPDF() // Use the same PDF function which opens print dialog
  }
                ...
                <CardContent className="pt-0">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1">
                      ...
                      <p className="text-base font-semibold text-slate-900">{student.moduleCode}</p>
                    </div>
                    <div className="space-y-1">
                      <p className="text-xs font-medium text-slate-500 uppercase tracking-wide">Module Name</p>
                      <p className="text-base font-semibold text-slate-900 truncate">{student.moduleName || 'N/A'}</p>
                    </div>
                    <div className="space-y-1">
                    ...
                    <div className="space-y-1">
                      <p className="text-xs font-medium text-slate-500 uppercase tracking-wide">Campus</p>
                      <p className="text-base font-semibold text-slate-600">{student.campus || 'N/A'}</p>
                    </div>
                    <div className="space-y-1">
